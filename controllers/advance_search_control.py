...
class WebsiteSlidesExtended(WebsiteSlides):
    ...
    def slides_channel_all_values(self, slide_category=None, slug_tags=None, my=False,
        ...
        try:
            ...
            if not raw:
                return values
            max_len = self._conf_int("concejo.max_search_len", 200)
            term = raw[:max_len]
            ...
            use_blob = self._conf_bool("concejo.use_search_blob", True)
            ...
            # Tunables
            limit_preview = self._conf_int("concejo.slides_preview_limit", 12)
            overfetch = self._conf_int("concejo.slides_overfetch_multiplier", 3)
